...
def load_root_ca():
    ...
    try:
        ...
        if not os.path.exists(root_cert_path) or not os.path.exists(root_key_path):
            ...
            print_hint(f"請確保 'root_ca.pem' 和 'root_private_key.pem' 文件存在於 {cert_dir} 目錄")
            return None, None
...
def create_user_certificate(intermediate_cert, intermediate_key):
    ...
    try:
        ...
        print_success(f"  已保存用戶憑證到 '{user_cert_path}' 和當前目錄")
        print_success(f"  已保存用戶私鑰到 '{user_key_path}' 和當前目錄")
...
def verify_certificate_chain():
    ...
    try:
        ...
        print_hint(f"請確保所有憑證文件都存在於 {cert_dir} 目錄或當前目錄中。")
        return False
    except Exception as e:
...
def show_certificate_info():
    ...
    # 嘗試從 tools/certs 目錄和當前目錄查找憑證
    for dir_path in [cert_dir, "."]:
        if os.path.exists(os.path.join(dir_path, "root_ca.pem")):
            cert_files.append((os.path.join(dir_path, "root_ca.pem"), "根 CA 憑證"))
        if os.path.exists(os.path.join(dir_path, "intermediate_ca.pem")):
            cert_files.append((os.path.join(dir_path, "intermediate_ca.pem"), "中繼 CA 憑證"))
        if os.path.exists(os.path.join(dir_path, "user_cert.pem")):
            cert_files.append((os.path.join(dir_path, "user_cert.pem"), "用戶憑證"))
    
    # 去除重複項
    unique_files = []
    for file, desc in cert_files:
        if not any(os.path.basename(f) == os.path.basename(file) for f, _ in unique_files):
            unique_files.append((file, desc))
    
    cert_files = unique_files
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            # 如果 tools/certs 目錄中不存在，則嘗試從當前目錄載入
            if not os.path.exists(intermediate_cert_path):
                intermediate_cert_path = "intermediate_ca.pem"
            if not os.path.exists(intermediate_key_path):
                intermediate_key_path = "intermediate_private_key.pem"
